...
var ping : float;
var jitter : float;
var localPlayer : GameObject;		//The "Player" GameObject for which this game instance is authoritative. Used to determine 
                                    // if we should be calculating physics on the object this script is controlling, in case it
                                    // could be colliding with this game instance's "player"
...
function Start() {
	...
	transform.localScale = Vector3.one; // WHY THIS HACK IS NEEDED?
}
...
function FixedUpdate() {
	...
	var absDiff : float = ping - (Network.time - states[0].t);
	jitter = Mathf.Lerp(jitter, Mathf.Abs(absDiff), Time.deltaTime * .3);
	...
	if (states[0].t > interpolationTime) {												// Target playback time should be present
																						// in the buffer
		var i : int;
		for (i=0; i<stateCount; i++) {													// Go through buffer and find correct state
																						// to play back
			if (states[i] && (states[i].t <= interpolationTime || i == stateCount-1)) {
				var rhs : State = states[Mathf.Max(i-1, 0)];							// The state one slot newer than the best
																						// playback state
				var lhs : State = states[i];											// The best playback state (closest to .1
																						// seconds old)
				var l : float = rhs.t - lhs.t;											// Use the time between the two slots to
																						// determine if interpolation is necessary
				var t : float = 0.0;													// As the time difference gets closer to
																						// 100 ms, t gets closer to 1 - in which
																						// case rhs is used
...
function OnSerializeNetworkView(stream : BitStream, info : NetworkMessageInfo) {
	...
	if(stream.isWriting) {
		...
		p = states[0].p;
		r = states[0].r;
		m = (Network.time - states[0].t) * 1000;	//m is the number of milliseconds that transpire between
													// the packet's original send time and the time it is resent
													// from the server to all the other clients
		stream.Serialize(p);
		stream.Serialize(r);
		stream.Serialize(m);
	}
	...
	//New packet recieved - add it to the states array for interpolation!
		...
		if(stateCount == 0) states[0] = state;
			...
			for (var k : int = states.Length-1; k > 0; k--) {
				states[k] = states[k-1];
			}
			states[0] = state;
		}
